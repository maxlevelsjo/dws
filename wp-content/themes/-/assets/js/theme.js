...
window.liquidElements = $ => {
  ...
  window.$liquidSections = $liquidTitlebar.add($liquidSectionsWrapper.find(elementorSectionsSelector)).add($elementorFooterSections);
...
(function ($) {
  ...
  class Plugin {
    ...
    getStickyElements() {
      const $stickyModules = this.DOM.$element.find('> .elementor:not(.lqd-mobile-sec)').find('[data-element_type="widget"]').filter((i, el) => {
  ...
  class Plugin {
    ...
    initSolidCircles(selectors, elementSetActiveClassname) {
      ...
      const init = item => {
        ...
        item.addEventListener("mouseleave", solidCircleLeave);
      };
      if (typeof selectors === 'string') {
        document.addEventListener("mousemove", event => {
          if (event.target.matches(`${selectors}, ${selectors} *`)) {
            this.activeStyle !== 'solid' && solidCircleEnter();
          } else {
            this.activeStyle === 'solid' && solidCircleLeave();
          }
        });
      } else {
        init(selectors);
    ...
    initArrowCursor(selectors) {
      ...
      if (typeof selectors === 'string') {
        document.addEventListener("mousemove", event => {
          if (event.target.matches(`${selectors}, ${selectors} *`)) {
            this.activeStyle !== 'arrow' && onEnter();
          } else {
            this.activeStyle === 'arrow' && onLeave();
          }
        });
      } else {
        init(selectors);
...
jQuery(document).ready(function ($) {
  if (window.document.documentElement.classList.contains('elementor-html')) {
    return;
  }
...
(function ($) {
  ...
  function Plugin(element, options) {
    ...
    this.beforeResizeQueue = [];
    this.afterResizeQueue = [];
  ...
  Plugin.prototype = {
    ...
    doOnWindowResize() {
      if (this.windowWidth !== liquidWindowWidth()) return;
      this.windowWidth = liquidWindowWidth();
      this.fullwidthSide();
      this.columnsAutoWidth();
    },
    ...
    parallax() {
      ...
      this.flickityData.cells.forEach((cell, i) => {
        var t = "left" == cell.parent.originSide ? "marginLeft" : "marginRight";
        const target = cell.x + cell.size[t] + cell.size.width * cell.parent.cellAlign;
        ...
        if (i === 0) {
          x = Math.abs(flkty.x) > flkty.slidesWidth ? flkty.slidesWidth + flkty.x + flkty.slides[flkty.slides.length - 1].outerWidth + target : target + flkty.x;
        } else if (i === flkty.slides.length - 1) {
          x = Math.abs(flkty.x) + flkty.slides[i].outerWidth < flkty.slidesWidth ? target - flkty.slidesWidth + flkty.x - flkty.slides[i].outerWidth : target + flkty.x;
        } else {
          x = target + flkty.x;
        }
        if ($cellImage.is(':only-child')) {
          $cellImage.css({
            ...
            top: '-1%',
            insetInlineStart: '-10%',
            'transform': `translate3d(${x * (multiply / 3)}px,0,0)`
          });
        }
      });
    },
      ...
      $controlledCarousels.each((i, controlledCarousel) => {
        ...
        if (controlledCarouselData) {
          controlledCarouselData.carouselInitPromise.then(() => {
            ...
            this.flickityData.on('settle', i => {
              syncing = false;
            });
            this.flickityData.on('dragMove', i => {
              syncing = false;
            });
            this.flickityData.on('scroll', progress => {
              if (syncing) return;
              const xVal = progress * (controlledCarouselData.flickityData.slideableWidth - controlledCarouselData.flickityData.size.width);
              syncing = true;
              controlledCarouselData.flickityData.x = -xVal;
              controlledCarouselData.flickityData.settle(-xVal);
              syncing = false;
            });
            controlledCarouselData.flickityData.on('change', i => {
              this.flickityData.select(i);
            });
            controlledCarouselData.flickityData.on('settle', i => {
              syncing = false;
            });
            controlledCarouselData.flickityData.on('dragMove', i => {
              syncing = false;
            });
            controlledCarouselData.flickityData.on('scroll', progress => {
              if (syncing) return;
              const xVal = progress * (this.flickityData.slideableWidth - this.flickityData.size.width);
              syncing = true;
              this.flickityData.x = -xVal;
              this.flickityData.settle(-xVal);
              syncing = false;
            });
          });
        }
  ...
  class Plugin {
    ...
    resizeWindow() {
      ...
      $(document).on('lqd-header-sticky-change', () => {
        if (this.$element.is(':visible') && this.$element.closest('.main-header').length) {
          onResize();
        }
      });
    }
...
jQuery(document).ready(function ($) {
  $('.megamenu').filter((i, el) => {
    ...
    return !$el.closest('.navbar-fullscreen').length && !isInMobileNav && !$el.parent().hasClass('lqd-menu-items-block');
  }).liquidMegamenu();
});
(function ($) {
  'use strict';
  ...
  class Plugin {
    ...
    asSectionBg() {
      if (this.options.asBG) {
        ...
        let parentSection = this.$element.parents('.elementor-section').last();
...
(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      if ($liquidMainHeader.find(this.$element).length && $liquidMainHeader[0].hasAttribute('data-sticky-header')) {
        $(document).on('lqd-header-sticky-change', () => {
    ...
    pin() {
      ...
      fastdom.mutate(() => {
        ...
        const transitionProps = this.$element.css('transition-property');
        const transitionPropsWithoutTransform = transitionProps.split(',').map(prop => prop.trim()).filter(prop => prop !== 'transform').join(',');
        ...
        mm.add(breakpoint, () => {
          this.ST = ScrollTrigger.create({
            ...
            pinReparent,
            onEnter: () => {
              this.$element.css('transition-property', transitionPropsWithoutTransform);
            },
            onEnterBack: () => {
              this.$element.css('transition-property', transitionPropsWithoutTransform);
            },
            onLeave: () => {
              this.$element.css('transition-property', null);
            },
            onLeaveBack: () => {
              this.$element.css('transition-property', null);
            }
    ...
    events() {
      $(document).on('lqd-header-sticky-change lqd-masonry-layout-complete lqd-carousel-initialized', this.refresh.bind(this));
    }
    refresh() {
    ...
    destroy() {
      this.ST && this.ST.revert(true);
      $(window).off('resize', this.onWindowResize);
      $(document).off('lqd-header-sticky-change lqd-masonry-layout-complete lqd-carousel-initialized', this.refresh.bind(this));
      $.data(this.element, "plugin_" + pluginName, null);
    }
  }
  $.fn[pluginName] = function (options) {
  ...
  class Plugin {
    ...
    init() {
      ...
      imagesLoaded(this.element, () => {
        ...
        this.$element.css('--hidden-el-height', `${this.hiddenElementHeight}px`);
    ...
    eventListeners() {
      ...
      this.$triggerElement.on('mouseleave.lqdSlideElementOnLeave', this.onMouseLeave.bind(this));
      $(document).on('click.lqdSlideElementOnClick', this.onClick.bind(this));
    ...
    onClick(event) {
      if (!this.$triggerElement[0].contains(event.target) && this.play) {
        this.onMouseLeave();
      }
    }
    onWindowResize() {
    ...
    destroy() {
      this.$triggerElement.off('mouseenter.lqdSlideElementOnHover mouseleave.lqdSlideElementOnLeave');
      $(document).off('click.lqdSlideElementOnClick');
...
(function ($) {
  ...
  class Plugin {
    ...
    onClick(event) {
      ...
      window.document.documentElement.style.scrollBehavior = 'auto';
      $('html, body').animate({
        scrollTop: y - localscrollGlobalOffset - this.offset
...
jQuery(document).ready(function ($) {
  const $elements = $('[data-split-text]').filter((i, el) => {
    const $el = $(el);
    const closestCustomAnimation = $el.closest('[data-custom-animations]');
    const closestAccordion = $el.closest('.accordion-content');
    const closestTab = $el.closest('.lqd-tabs-pane');
    const closestWebglSlideshow = $el.closest('[data-lqd-webgl-slideshow]');
    return !closestCustomAnimation.length && !closestAccordion.length && !closestTab.length && !closestWebglSlideshow.length;
  });
  ...
  class Plugin {
    ...
    _getInnerChildElements(elements) {
      ...
      $.each($elements, (i, element) => {
        ...
        if (!element.classList.contains('ld-empty-space') && !element.classList.contains('ld-particles-container') && !element.classList.contains('elementor-widget-spacer') && !element.hasAttribute('data-split-text') && element.tagName !== 'STYLE') {
  ...
  class Plugin {
    ...
    async init() {
      ...
      fastdomPromised.mutate(() => {
        ...
        this.parallaxTimeline.fromTo(this.parallaxElement, {
          ...this.parallaxFromOptions
        }, {
          ease,
          ...this.parallaxToOptions
        ...
        this.ST = ScrollTrigger.create({
          ...
          onUpdate: () => {
            this.$element.css('transition-property', transitionPropsWithoutTransform);
            this.switchWillChange('on');
          },
          onLeave: () => {
            this.$element.css('transition-property', null);
            this.switchWillChange('off');
          },
          onLeaveBack: () => {
            this.$element.css('transition-property', null);
            this.switchWillChange('off');
...
jQuery(document).ready(function ($) {
  $('[data-parallax]').not('.rev-slidebg').liquidParallax();
});
(function ($) {
  'use strict';
